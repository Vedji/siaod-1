...
struct List {
    ...
    Node* Given();
    void Insert();
    vector<int> Remove(vector<int> nodes, vector<Node*>* nodes_ptr);
    void Delete(vector<int> Remove);
    void new_function_delete(vector<Node*> code);
...
vector<int> List::Remove(vector<int> nodes, vector<Node*>* nodes_ptr) {
    Node* ex = Head;
    int count = 0;
    vector<int> codes;
    vector<int> means;
...
void List::new_function_delete(vector<Node*> code) {
    for (Node* i : code) {
        if (i->prev == nullptr && i->next == nullptr) {
            Tail = nullptr;
            Head = nullptr;
        }
        else if (i->next != nullptr && i->prev != nullptr) {
            i->prev->next = i->next;
            i->next->prev = i->prev;
        }
...
void List::Delete(vector<int> Remove) {
    for (int i = 0; i < Remove.size(); i++) { // Тут ты начинаешь проходить по узлам
        Node* p = Head;
        Node* save;
        Node* last;
        while (p != nullptr) {     // Однако тут идет перебор начиная с узла i до конца списка 
            if (p->code == Remove[i]) {
                if (p->prev == nullptr) {
                    tired++;
                    Head = p->next;
...
int main()
{
    ...
    // m.Delete();
    m.Remove(pArr, &node_ptrs);
    m.new_function_delete(node_ptrs);
    m.Show();
